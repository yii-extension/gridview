...
namespace Yii\Extension\GridView\Widget\Bulma;
...
final class LinkPager extends Widget
{
    ...
    /**
     * @var array HTML attributes which will be applied to all button containers,
     */
     ...
     */
    private string $prevPageCssClass = 'pagination-previous';

    /**
     * @var string the CSS class for the "next" page button.
     */
    private string $nextPageCssClass = 'pagination-next';
    ...
    protected function renderPageButtons(): string
    {
        ...
        if ($firstPageLabel !== null) {
            ...
            $linkOptions['data-page'] = 0;

            Html::addCssClass($linkOptions, $this->firstPageCssClass);

            $links[] = Html::a($firstPageLabel, $this->pagination->createUrl(0), $linkOptions);
        }
        ...
        if ($this->prevPageLabel !== null) {
            ...
            Html::addCssClass($linkOptions, $this->prevPageCssClass);

            $links[] = Html::a($this->prevPageLabel, $this->pagination->createUrl($page), $linkOptions);
        }
        ...
        if ($this->nextPageLabel !== null) {
            ...
            Html::addCssClass($linkOptions, $this->nextPageCssClass);

            $links[] = Html::a($this->nextPageLabel, $this->pagination->createUrl($page), $linkOptions);
        }

        /* link last page */
        $lastPageLabel = $this->lastPageLabel !== null ? (string) $pageCount : $this->lastPageLabel;

        if ($lastPageLabel !== null) {
            $linkOptions = $this->linkOptions;
            $linkOptions['data-page'] = $pageCount - 1;

            Html::addCssClass($linkOptions, $this->lastPageCssClass);

            $links[] = Html::a($lastPageLabel, $this->pagination->createUrl($pageCount - 1), $linkOptions);
        }
